...
data "aws_secretsmanager_secret" "website-secrets" {
  arn = "arn:aws:secretsmanager:eu-central-1:218885890069:secret:staging/penguin-exchange/backend-IScjZX"
}
...
module "task-website" {
  ...
  docker_labels = {
    ...
    "traefik.http.routers.website.rule"                        = "Host(`${var.project_env}.${var.project_domain}`) && PathPrefix(`/`)",
...
#    "command" : [
#      "CMD-SHELL",
#      "curl -f -k http://localhost:3000/en || exit 1"
#    ],
#    "timeout" : 5,
#    "interval" : 30,
#    "startPeriod" : 5
